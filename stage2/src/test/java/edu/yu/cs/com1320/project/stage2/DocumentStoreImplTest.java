...
public class DocumentStoreImplTest {
    ...
    @Test
    public void testUndoAddingNewDoc() throws IOException {
        DocumentStore store = new DocumentStoreImpl();
        store.putDocument(new ByteArrayInputStream(this.txt1.getBytes()), this.uri1, DocumentStore.DocumentFormat.TXT);
        assertEquals(this.txt1, store.getDocument(this.uri1).getDocumentTxt());
        store.undo(this.uri1);
        assertEquals(null, store.getDocument(this.uri1));
    }

    @Test
    public void testUndoDeleteDoc() throws IOException {
        DocumentStore store = new DocumentStoreImpl();
        store.putDocument(new ByteArrayInputStream(this.txt1.getBytes()), this.uri1, DocumentStore.DocumentFormat.TXT);
        assertEquals(this.txt1, store.getDocument(this.uri1).getDocumentTxt());
        store.deleteDocument(this.uri1);
        ...
        store.undo();
        assert store.getDocument(this.uri1) != null : "should be a doc stored";
    }

    @Test
    public void testReplaceDoc() throws IOException {
        //put the first version
        DocumentStore store = new DocumentStoreImpl();
        int returned = store.putDocument(new ByteArrayInputStream(this.txt1.getBytes()),this.uri1, DocumentStore.DocumentFormat.TXT);
        assertTrue(returned == 0);
